...
$result = mysqli_query($conn_products, $sql);
...
$sql_images = "SELECT * FROM product_images WHERE product_id = $id ORDER BY id ASC";
$images_result = mysqli_query($conn_products, $sql_images);
$images = [];
...
<body class="layout-fixed fixed-header sidebar-expand-lg sidebar-open bg-body-tertiary">
                            ...
                            <div class="row g-0">
                                <div class="col-lg-6">
                                    ...
                                    <div class="gallery-container">
                                        <?php if (!empty($images)): ?>
                                            <img src="<?php echo htmlspecialchars($images[0]['image_path']); ?>" id="mainView" class="main-image shadow-sm" alt="Product">
                                ...
                                <div class="col-lg-6">
                                    <div class="p-4 p-lg-5">
                                        ...
                                        <div class="mb-4">
                                            <span class="price-large">₹<?php echo number_format((float)$product['price'], 2); ?></span>
                                        </div>

                                        <div class="row mb-4">
                                            <div class="col-6 border-end">
                                                <span class="text-muted d-block small text-uppercase fw-bold">Current Stock</span>
                                                <h4 class="<?php echo $product['stock'] > 10 ? 'text-success' : ($product['stock'] > 0 ? 'text-warning' : 'text-danger'); ?>">
                                                    <?php echo $product['stock'] > 0 ? $product['stock'] . ' Units' : 'Out of Stock'; ?>
                                                </h4>
                                            </div>
                                            <div class="col-6 ps-4">
                                                <span class="text-muted d-block small text-uppercase fw-bold">Added Date</span>
                                                <h4><?php echo date('M d, Y', strtotime($product['created_date'])); ?></h4>
                                            </div>
                                        </div>


                                        <div class="mb-3">
                                            <span class="badge-status <?php echo $product['status'] === 'active' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger'; ?>">
                                                <?php echo htmlspecialchars($product['status']); ?>
                                            </span>
                                        </div>

                                        <button type="submit" class="btn btn-success mt-4 btn-lg">Buy this product</button>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        function changeImage(src, el) {
            ...
            document.querySelectorAll('.thumb-image').forEach(img => img.classList.remove('active'));
